...
export default function SuperAdmin() {
  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-12 flex items-center justify-center gap-3">
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        {tabs.map(t => {
          ...
          return (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all shrink-0 ${active ? "bg-slate-800 text-white shadow-sm" : "bg-white text-slate-500 border border-slate-200 hover:bg-slate-50"}`}>
              <Icon className="w-4 h-4" /> {t.label}
              {t.key === "restaurants" && <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${active ? "bg-white/20" : "bg-slate-100"}`}>{restaurants.length}</span>}
            </button>
          );
        })}
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => { setTab("restaurants"); setShowAddForm(true); }}
                    ...
                    <button onClick={() => setTab("restaurants")}
                      ...
                      <Building2 className="w-4 h-4" /> View All
                    </button>
              ...
              {restaurants.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={() => setTab("restaurants")} className="text-xs text-sky-600 font-medium hover:text-sky-700">View all →</button>
                  </div>
                      ...
                      <button key={r.id} onClick={() => nav(`/super-admin/restaurant/${r.id}`)}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-[10px] text-slate-400">
                            ...
                            {r._count && <span>{r._count.orders} orders</span>}
                          </div>
                        </div>
          ...
          {tab === "restaurants" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-slate-800">Restaurants</h3>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          ...
                          <div>
                            ...
                            <input value={newRestaurant.email} onChange={(e) => { setNewRestaurant(s => ({ ...s, email: e.target.value })); setAddError(""); }}
                              type="email" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200" />
                          </div>
                          <div>
          ...
          {tab === "plans" && (
            <div className="space-y-4">
              <h3 className="font-bold text-slate-800">Subscription Plans</h3>

              {/* Add plan form */}
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">New Plan</p>
                <div className="flex gap-3 flex-wrap items-end">
                  <div className="flex-1 min-w-[160px]">
                    ...
                    <input value={newSub.name} onChange={(e) => { setNewSub((s) => ({ ...s, name: e.target.value })); setSubError(""); }}
                      placeholder="e.g. AI Basic" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200" />
                  </div>
                  <div className="w-28">
                    ...
                    <input type="number" value={newSub.price || ""} onChange={(e) => setNewSub((s) => ({ ...s, price: Number(e.target.value) }))}
                      placeholder="0" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200" />
                  </div>
                  <div className="w-28">
                    ...
                    <input type="number" value={newSub.maxTables} onChange={(e) => setNewSub((s) => ({ ...s, maxTables: Number(e.target.value) }))}
                      placeholder="20" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200" />
                  </div>
                      ...
                      {editSubId === s.id ? (
                        ...
                        <>
                          ...
                          <button onClick={() => { setEditSubId(s.id); setEditSub({ name: s.name, price: s.price, maxTables: s.maxTables }); }}
                            className="w-full text-xs font-semibold text-slate-500 bg-slate-50 py-2 rounded-xl hover:bg-slate-100 transition-colors">
                            Edit Plan
                          </button>
                        </>
                      )}
          ...
          {tab === "insights" && (
            <div className="space-y-4">
              <h3 className="font-bold text-slate-800">Questionnaire & Mood Insights</h3>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }}
            className="fixed bottom-6 right-4 bg-slate-800 text-white text-sm px-4 py-2.5 rounded-xl shadow-lg z-50"
          >{toast}</motion.div>
        )}
      </AnimatePresence>
    </div>
