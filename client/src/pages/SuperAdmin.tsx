        ...
        {stats.map((s) => {
          ...
          return (
            <div key={s.label} className="relative overflow-hidden bg-white rounded-xl border border-slate-200 p-3">
              <div className={`absolute -top-3 -right-3 w-12 h-12 rounded-full bg-gradient-to-br ${s.gradient} opacity-10`} />
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-white bg-gradient-to-br ${s.gradient} mb-1.5`}><Icon className="w-3.5 h-3.5" /></div>
            ...
            {showAddForm && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                <div className="mt-3 bg-slate-50 rounded-xl p-4 space-y-3">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">New Restaurant Details</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <input value={newRestaurant.name} onChange={(e) => { setNewRestaurant(s => ({ ...s, name: e.target.value })); setAddError(""); }}
                      placeholder="Restaurant name *" className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200 bg-white" />
                    <input value={newRestaurant.ownerName} onChange={(e) => { setNewRestaurant(s => ({ ...s, ownerName: e.target.value })); setAddError(""); }}
                      placeholder="Owner name *" className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200 bg-white" />
                    <input value={newRestaurant.email} onChange={(e) => { setNewRestaurant(s => ({ ...s, email: e.target.value })); setAddError(""); }}
                      placeholder="Owner email *" type="email" className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200 bg-white" />
                    <input value={newRestaurant.phone} onChange={(e) => setNewRestaurant(s => ({ ...s, phone: e.target.value }))}
                      placeholder="Phone (optional)" className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200 bg-white" />
                  </div>
                  ...
                  <button onClick={addRestaurant} disabled={addLoading}
                    ...
                    {addLoading ? "Creating..." : <><Plus className="w-4 h-4" /> Create Restaurant</>}
                  </button>
                </div>
              </motion.div>
            )}
            ...
            {createdCreds && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                <div className="mt-3 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                  ...
                  <p className="text-xs text-emerald-600 mb-3">The owner can reset their password after logging in.</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div className="bg-white rounded-lg p-3 border border-emerald-100">
                      <div className="text-[10px] font-bold text-slate-400 uppercase mb-1">Restaurant</div>
                      <div className="text-sm font-medium text-slate-800">{createdCreds.restaurantName}</div>
                    </div>
                    <div className="bg-white rounded-lg p-3 border border-emerald-100">
                      <div className="text-[10px] font-bold text-slate-400 uppercase mb-1">Owner</div>
                      <div className="text-sm font-medium text-slate-800">{createdCreds.ownerName}</div>
                    </div>
                    <div className="bg-white rounded-lg p-3 border border-emerald-100">
                      <div className="text-[10px] font-bold text-slate-400 uppercase mb-1">Email (Login)</div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-mono text-slate-800 truncate">{createdCreds.email}</span>
                        <button onClick={() => copyToClipboard(createdCreds.email)} className="p-1 rounded hover:bg-emerald-50"><Copy className="w-3 h-3 text-slate-400" /></button>
                      </div>
                    </div>
                    <div className="bg-white rounded-lg p-3 border border-emerald-100">
                      <div className="text-[10px] font-bold text-slate-400 uppercase mb-1">Temporary Password</div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-mono text-slate-800">{showPassword ? createdCreds.tempPassword : "••••••••"}</span>
                        <button onClick={() => setShowPassword(!showPassword)} className="p-1 rounded hover:bg-emerald-50">
                          {showPassword ? <EyeOff className="w-3 h-3 text-slate-400" /> : <Eye className="w-3 h-3 text-slate-400" />}
                        </button>
                        <button onClick={() => copyToClipboard(createdCreds.tempPassword)} className="p-1 rounded hover:bg-emerald-50"><Copy className="w-3 h-3 text-slate-400" /></button>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => { setCreatedCreds(null); setShowAddForm(false); }}
                    className="mt-3 text-xs font-medium text-emerald-700 hover:text-emerald-800">Dismiss</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        <div className="p-4 border-b border-slate-200">
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            ...
            <input type="number" value={newSub.maxTables} onChange={(e) => setNewSub((s) => ({ ...s, maxTables: Number(e.target.value) }))} placeholder="Table count" className="w-28 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200" />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1">
                      ...
                      <input type="number" value={editSub.price} onChange={(e) => setEditSub((p) => ({ ...p, price: Number(e.target.value) }))} className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200" />
                    </div>
                    <div className="w-20">
                      <label className="text-[9px] text-slate-400 font-bold uppercase">Tables</label>
                      <input type="number" value={editSub.maxTables} onChange={(e) => setEditSub((p) => ({ ...p, maxTables: Number(e.target.value) }))} className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-200" />
                    </div>
                  </div>
                  <div className="flex gap-1.5">
